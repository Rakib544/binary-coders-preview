        ...
        >
          <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='24' height='24'>
            {navShow ? (
              ...
              <>
                <path fill='none' d='M0 0h24v24H0z' />
                <path d='M3 4h18v2H3V4zm6 7h12v2H9v-2zm-6 7h18v2H3v-2z' fill='#fff' />
              </>
            )}
          </svg>
        </button>
        ...
        <nav className='fixed py-6 px-6  h-full w-full z-20 bg-[#111121]'>
          {username && (
            <Link to={`/user/${username}`} prefetch='intent' onClick={onToggleNav}>
              <div className='flex items-center space-x-4 border-b border-slate-300 pb-4'>
                ...
                />
                <div>
                  <p className='text-xl font-medium mt-2 text-slate-200'>{fullName}</p>
          ...
          {mobileNavLinks.map((link) => (
            <div key={link.title} className=' px-4 py-4'>
              <Link
                prefetch='intent'
                to={link.href}
                className={'text-lg font-medium tracking-widest text-slate-300'}
                onClick={onToggleNav}
              >
                {link.title}
              </Link>
          ...
          {!username ? (
            <Link
              prefetch='intent'
              to='/auth/login'
              className='px-8 sm:px-12 py-2 sm:py-3  bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600 transition duration-200  inline-block my-4'
              onClick={onToggleNav}
            >
              Login
            </Link>
            ...
            <>
              <div key={'setting'} className=' px-4 py-4 bg-[#111121]'>
                <Link
                  prefetch='intent'
                  to='/setting'
                  className={'text-lg font-medium tracking-widest text-slate-300'}
              ...
              <Form method='post' action='/auth/logout'>
                <button
                  type='submit'
                  className='px-8 sm:px-12 py-2 sm:py-3  bg-blue-500 text-white rounded-lg text-sm font-medium  hover:bg-blue-600 transition duration-200 inline-block my-4 border-2 border-blue-500 hover:border-blue-600'
                  onClick={onToggleNav}
                >
                  Logout
                </button>
